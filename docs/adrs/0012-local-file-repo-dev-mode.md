...
* Status：Accepted
* Date：2026-05-17
* 適用範圍：`make dev` 與 CI dev compose；**production 必須拒絕**
...
結果是 `make dev` 下：preview 過、confirm 寫 DB、`deploy_run` 卡在 `queued`，無人推進至 `live`。
...
* `make dev-create-example` 一鍵跑通 create_app → live；新貢獻者上手成本降至 < 5 分鐘
