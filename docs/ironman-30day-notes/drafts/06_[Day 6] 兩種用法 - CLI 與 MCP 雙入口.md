...
過去幾天我們建立了整體認識：[Day 2] 看了 AI 一句話部署的 demo、[Day 4] 拆了 preview/confirm 安全網、[Day 5] 和其他平台做了選型對照。你可能已經注意到，同一件「建立 app」的事，[Day 2] 是 agent 呼叫 MCP 工具做的，但 [Day 4]、[Day 5] 又出現了 `0ops apps create` 這種手打的指令。到底哪個才是「正宗用法」？

答案是：**兩個都是，而且它們走的是同一條路。**今天就把 0ops「一套後端、兩個入口」的設計講清楚。
...
回顧 [Day 1] 講的三個組成：`0ops-server`（後端）、`0ops`（CLI）、`0ops-mcp`（MCP server）。關鍵在於後兩者的關係——它們是同一個後端的兩張臉：
...
最直觀的理解方式是把幾個常見操作並排看。左邊是你手動打的 CLI，右邊是 agent 呼叫的 MCP 工具：
...
而在 Claude Code 裡，你只要問「我現在有哪些 app？」，agent 會替你呼叫 `list_apps`（帶上 team_slug），拿到同一份資料，再用自然語言回你。**同一份後端資料，兩種取用方式。**
...
而且這不是二選一。你完全可以早上用 CLI 寫腳本、下午在 Claude Code 裡對話部署，兩邊操作的是同一批 app、同一個團隊、同一份狀態。

關鍵：換入口不換保證

這是今天最重要的一點。因為兩個入口打到的是同一個後端，所以：
...
今天我們把 0ops 的雙入口模型講清楚了：`0ops` CLI 給人手動用、`0ops-mcp` 給 AI agent 呼叫，兩者打到同一個後端。同一件事兩種做法（`apps list` vs `list_apps`），24 個 MCP 工具和 CLI 動作一一對應；探索/腳本用 CLI、對話式開發用 MCP。最關鍵的是——換入口不換保證，權限與 preview/confirm 兩條路完全一致。

到這裡，第一章的「觀念地基」就鋪完了：你知道 0ops 是什麼、誰該用、安全網怎麼運作、和別人怎麼比、有哪兩個入口。明天 [Day 7] 開始動真格——一條 curl 指令，三分鐘把 0ops 裝好、登入、接上你的 AI CLI。

Q&A

你猜自己之後會偏用哪個入口多一點，CLI 還是 MCP？還是看場合混著用？歡迎留言聊聊你的工作流 : )
