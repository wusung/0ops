...
[Day 2] 的 demo 裡，agent 在真正建立 app 之前停下來、把計畫攤給你看、等你點頭；[Day 3] 筆者陪大家判斷了誰該用 0ops。今天想回答一個筆者自己一開始也很在意的核心問題：**你憑什麼敢讓一個 AI 去執行部署，甚至去刪除正式環境？**

老實說，這也是筆者剛接觸 agent 部署時最不安的地方。而讓筆者慢慢放下心的，就是這道安全網：preview/confirm（預覽/確認）。它是 0ops 讓人敢放手的關鍵設計，也是整個平台的招牌保證。

今天大概會一起搞懂三件事：

- 為什麼所有「寫入操作」都被拆成兩階段；
- 在 CLI 手動操作時，實際會被要求打什麼（尤其是刪除）；
- 透過 AI agent 操作時，那道閘怎麼運作、為什麼 agent 繞不過。
...
你看意圖、看代價，覺得可以接受，才進到 confirm。筆者覺得這個設計的精神很簡單：**讓你在付出代價之前，先看清楚代價。**
...
打 `y` 才執行。如果你在腳本裡不想被卡住，可以加 `--yes` 跳過這個確認；或者加 `--dry-run` 只看 preview、完全不執行。筆者覺得這是最輕量的一道閘——因為建立 app 是可回復的（大不了刪掉重來）。
...
如果你加 `--yes`，只會跳過**最後**那道 `[y/N]`；前面「打 slug」和「打 required_phrase」的關卡仍然在。筆者理解這是刻意的——不可逆操作不給你一個旗標全部跳過。
...
筆者覺得關鍵在於：**這道閘是後端強制的，不是 agent 自律的。**

很多人（包括筆者一開始）會以為「AI 安不安全」取決於 prompt 寫得好不好、agent 乖不乖。但 0ops 的設計並不賭這個。就算你對 agent 說「不要問我，全部自動執行」，agent 頂多能連續呼叫 preview 和 confirm——但它**沒有一個「跳過 preview 直接寫入」的工具可以呼叫**。寫入能力只存在於 confirm 工具，而 confirm 工具一定要有效的 `preview_id` 才動。

這就是 [Day 3] 說的那個判斷原則的落地：**讓 AI 有權執行，但無權略過人的確認。**權力和確認被拆開，放在不同的地方，agent 只拿到前者。

總結

今天筆者跟大家一起拆開了 0ops 的招牌安全網：所有寫入操作都是 preview（看意圖＋看代價）→ confirm（帶 preview_id 執行）兩階段。CLI 上，create 是一道 `[y/N]`，delete 是「打 slug → 打 `DELETE <slug>` → `[y/N]`」三道閘；AI 上，agent 必須先展示 side_effects 取得同意、confirm 必帶有效 preview_id、delete 還要傳對 `confirmation_phrase`。而這一切都是後端強制的，agent 繞不過。

理解了這道閘，大概就理解了「為什麼敢放手」。明天 [Day 5]，筆者想把 0ops 放到擂台上，和 Vercel、Railway、自建 K8s 做一次從約束出發的選型對照。

Q&A

你覺得刪除要打三次確認會不會太囉唆呢？還是反而希望正式環境的刪除門檻越高越好？筆者自己是偏好門檻高一點啦，但也很好奇大家的想法，歡迎留言唷 : )
