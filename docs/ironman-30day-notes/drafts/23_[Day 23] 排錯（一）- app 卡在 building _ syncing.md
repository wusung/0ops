...
到 [Day 22] 為止，「正常路徑」筆者已經陪大家全部走完了：裝好、接上 AI CLI、部署、看 log、redeploy、綁網域、管團隊、查稽核。但筆者自己用久了很清楚，真實世界不會永遠 happy path。今天起連兩天，筆者想跟大家聊排錯，主題是幾乎每個人遲早都會遇到的畫面——**你建了一個 app，狀態卻卡在 `building` 或 `syncing`，遲遲不變 `live`。**

先分享一個筆者自己踩過教訓才學會的心態：0ops 底層是一套會自癒的系統（reconciler + GitOps）。很多「卡住」其實只是還沒收斂，你一慌就手動介入，反而打亂它的收斂週期。所以排錯的第一原則是——**先等一個收斂週期，再動手**。
...
明天 [Day 24] 是排錯篇二，筆者要處理兩種更棘手的狀況：app 卡在 `deleting` 刪不乾淨，以及部署明明成功了、**網址卻打不開**。後者要從 Cloudflare 一路查到 pod，筆者會帶大家逐層定位。

Q&A

你遇過最久的一次「卡住」是幾分鐘？最後是等到收斂，還是動手介入才解決的？筆者自己也還在累積各種奇怪的卡關案例，很歡迎你留言分享排錯經驗給我唷 : )
