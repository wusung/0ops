...
昨天 [Day 1] 我們把場景講清楚了：AI coding agent 讓開發變快了十倍，但「出貨」這最後一哩還是斷的，而 0ops 想補的就是這一哩。理論講完，今天不囉唆，直接看一個 30 秒的 demo——在 Claude Code 裡打一句話，看它怎麼把一個 GitHub repo 部署上線。

今天你會看到三件事：

- 在對話框打一句自然語言，agent 背後跑了哪些 MCP 工具；
- 那個「你點頭才執行」的 preview 是長什麼樣子；
- 部署完拿到一個 `<你的 app>.jesontech.com` 的網址，狀態怎麼從 building 轉成 live。

先講清楚：這篇是「先看結果」。中間每一個工具、每一道確認閘，後面幾天都會拆開細講——今天的目的是讓你先感受一次「意圖直接變結果」是什麼體驗。

你想做什麼

假設你手上有一個 Next.js 的 repo，已經在本機跑得好好的，現在你只想把它丟上線，讓別人能用網址打得開。傳統上你要 build image、推 registry、寫 K8s manifest、設 ingress、掛網域、接 TLS——十幾步，全手工。

今天我們換一種做法：打開已經接好 0ops 的 Claude Code（怎麼接，[Day 8] 會教），直接對它說話。
...
就這樣。接下來的事情，agent 會自己接手，但它不會偷偷做完——它會停在關鍵的那一步等你點頭。
...
重點在中間那一步 `create_app_preview`。這是一個**唯讀**的預覽工具，它不會真的建立任何東西，只會回一份「如果你批准，接下來會發生什麼」的清單。agent 拿到這份清單後，必須先把它秀給你看、取得你明確的同意，才能呼叫真正會寫入的 `create_app`。

這道閘是後端強制的，agent 繞不過去——這正是你敢放手讓 AI 執行部署的原因。（完整的 preview/confirm 機制，明天 [Day 4] 專篇細講。）
...
你讀完覺得沒問題，回一句「好，執行」，agent 才會呼叫 `create_app`。
...
拿到 `subdomain_url` 不代表馬上就通——build 和上線需要一點時間。agent 會接著輪詢 `get_deploy_status`，你會看到狀態一路從 `building`（正在打包映像）走到 `syncing`（GitOps 正在同步到叢集），最後收斂成 `live`：
...
同樣一件事，如果純手工，你大概要做這些：寫 Dockerfile、`docker build`、`docker push` 到 registry、寫 Deployment / Service / Ingress 三份 YAML、設定 TLS 憑證、在 DNS 加一筆記錄、`kubectl apply`、然後盯著 pod 看它有沒有起來。中間任何一步打錯，你都要回頭 debug。
...
今天你看到了 0ops 的招牌體驗：一句自然語言，經過 `create_app_preview` → 你點頭 → `create_app` → 輪詢到 `live`，就把一個 repo 送上線，還配了一個網址。中間十幾步的部署儀式，被工具整個吸收掉了。

但你可能已經注意到，整個 demo 最關鍵的其實是那道「你點頭才執行」的閘。明天 [Day 3]，我們先退一步，聊聊「誰該用 0ops、什麼情況最適合」——先幫你判斷這套工具是不是對症下藥，再往下深入。

Q&A

你平常都用哪套 AI CLI？如果它能一句話幫你部署，你最想先丟哪個 repo 上去？歡迎留言聊聊 : )
