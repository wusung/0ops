...
1. **你想做什麼**：一個具體任務或痛點（「把這個 repo 部署上線」「卡在 building 怎麼辦」）。
2. **動手做**：可複製的指令 / MCP 工具呼叫 / 設定步驟，逐步。
3. **你會看到什麼**：實際輸出、確認提示、URL，讓讀者對齊預期。
4. **踩坑與要點**：常見錯誤與一條實務判斷。
5. **明日預告**：一句銜接。

- **長度**：每篇 1,500–2,500 字，至少一段可執行指令或一張流程圖（Mermaid）。
- **語言**：正體中文（台灣用詞），技術名詞與指令保留英文原文。
- **雙語選項**：標題與 TL;DR 可加英文，供日後 syndication；正文中文。

## 章節結構（30 篇分五類）

| 類別 | Day | 主題 |
|---|---|---|
| 介紹 | 1–4 | 0ops 是什麼、30 秒價值、誰該用、agent 出貨的安全網 |
| 解決方案 | 5–9 | vs 其他 PaaS、CLI+MCP 雙路徑、安裝上手、接 AI CLI、工具授權 |
| 操作 | 10–18 | 部署 app、看 log、redeploy、綁網域、團隊、生命週期、CI token |
| 實務 | 19–25 | AI 端到端部署、preview/confirm 實戰、push-to-deploy、權限與稽核、排錯 |
| 進階 | 26–30 | self-host、生產 OAuth/網域、SSO、audit/合規、運維與回顧 |
