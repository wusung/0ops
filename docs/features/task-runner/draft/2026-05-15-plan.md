...
**Goal:** Build a local-only task runner (`./manage.sh task-run-all` / `./manage.sh task-run <ID>` / `./manage.sh task-rerun <ID>`) that drives a git-worktree-isolated agent CLI through the AGENTS.md Mandatory Loop, verifies, auto-commits, opens a PR, and merges to main.

**Architecture:** Pure bash under `tasks/run/`. Two markdown tables (`tasks/task-list.md`, `tasks/task-status.md`) are the only source of truth. Each task gets its own `git worktree` + `task/<ID>` branch; the agent's prompt embeds the full Mandatory Loop checklist. Verify is a 3-section gate (status diff, expected paths, `./manage.sh test`). Push / PR / merge use `gh`. Resume is implicit: a leftover worktree on a `Pending` task is treated as resumable.
...
**Aggregator:** `./manage.sh task-runner-test` runs `tasks/run/test/run-tests.sh` which iterates `test_*.sh` and reports pass/fail count.
...
# Stub Makefile so `./manage.sh test` succeeds inside the worktree.
printf 'test:\n\t@echo ok\n' >Makefile
...
task-run: ## 跑指定 task：./manage.sh task-run M2.5
	@test -n "$(TASK)" || (echo "usage: ./manage.sh task-run <ID>" >&2; exit 1)
	@bash tasks/run/run-one.sh $(TASK)

task-rerun: ## 強制重跑指定 task：./manage.sh task-rerun M2.5
	@test -n "$(TASK)" || (echo "usage: ./manage.sh task-rerun <ID>" >&2; exit 1)
...
Run: `./manage.sh task-runner-test`
Expected: `Task runner tests: 11 passed, 0 failed`.

- [ ] **Step 3: Smoke test `./manage.sh task-list`**

Run: `./manage.sh task-list`
...
- [ ] **Step 4: Smoke test `./manage.sh task-next`**

Run: `./manage.sh task-next`
...
2. Run `./manage.sh task-run M0.99` with `TASK_SKIP_CI_WAIT=1` (CI not wired yet).
